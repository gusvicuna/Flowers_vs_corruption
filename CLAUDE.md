...
- **Health** (the only stat that kills you): drains continuously while the player stands on a corrupted tile (any phase), slowly while hunger is at 0, and in a lump when eating a corrupted crop. The two continuous drains **stack**. Eating restores some health back. At 0 → `PlayerDied`, the clock stops, and the future Game Over screen listens.
- **Hunger**: drains continuously through the whole cycle; refilled by *manually* eating a crop from the inventory (deliberate choice: eat vs replant/use). Hunger at 0 does not kill on its own — it bleeds health instead.
- **Vitals contract**: `PlayerVitals.Consume(hungerRestored, healthRestored, healthDamage)` is the single entry point for eating — the inventory calls it with the values from the eaten `CropDefinition` (`HungerRestored`/`HealthRestored`/`HealthDamage`). The vitals system knows nothing about inventory, so both can be built in parallel.
